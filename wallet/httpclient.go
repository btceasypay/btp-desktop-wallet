...
// NewHtpc make btpd http client
func NewHtpc() (*httpConfig, error) {

	if config.Cfg.BtpdSelect != "" {
		var btpd *client.Config
		for _, item := range config.Cfg.Btpds {
			if item.Name == config.Cfg.BtpdSelect {
				btpd = item
				break
			}
		}
		if btpd != nil {
			return NewHtpcByCfg(btpd)
		}
	}
