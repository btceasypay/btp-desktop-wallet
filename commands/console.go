...
import (
	...
	"github.com/btceasypay/btp-desktop-wallet/rpc/walletrpc"
	"github.com/btceasypay/btp-desktop-wallet/util"
	waddrmgr "github.com/btceasypay/btp-desktop-wallet/waddrmgs"
	"github.com/btceasypay/btp-desktop-wallet/wallet"
	"github.com/btceasypay/btp-desktop-wallet/wtxmgr"
...
var ConsoleCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if b == false {
			fmt.Println("Please create a wallet first,[btp-wallet bc create ]")
			return
		}
		startConsole()
	},
...
func OpenWallet() error {
	...
	if b {
		...
		return fmt.Errorf("Please create a wallet first,[btp-wallet bc create ]")
	}
	return nil
}
...
func createNewAccount(arg string) error {
	cmd := &btpjson.CreateNewAccountCmd{
		Account: arg,
	}
...
func getBalance(addr string) (*wallet.Balance, error) {
	cmd := &btpjson.GetBalanceByAddressCmd{
		Address: addr,
	}
...
func getListTxByAddr(addr string, filter int, pageNo int, pageSize int) (interface{}, error) {
	helper = &JsonCmdHelper{
		JsonCmd: &btpjson.GetListTxByAddrCmd{
			Address:  addr,
			Stype:    int32(filter),
			Page:     int32(pageNo),
...
func getBillByAddr(addr string, filter int, pageNo int, pageSize int) (interface{}, error) {
	helper = &JsonCmdHelper{
		JsonCmd: &btpjson.GetBillByAddrCmd{
			Address:  addr,
			Filter:   int32(filter),
			PageNo:   int32(pageNo),
			PageSize: int32(pageSize),
...
func getAddressesByAccount(account string) (interface{}, error) {
	...
	cmd := &btpjson.GetAddressesByAccountCmd{
		Account: account,
	}
...
func seedToAddr(seed string, network string) (string, error) {
	pri, err := bx.EcNew("secp256k1", seed)
	if err != nil {
		return "", err
	}
...
func priToPub(pri string, uncompressed bool) (string, error) {
	msg, err := bx.EcPrivateKeyToEcPublicKey(uncompressed, pri)
	if err != nil {
		return "", err
	}
...
func pubToAddr(pub string, net string) (string, error) {
	...
	msg, err := bx.EcPubKeyToAddress(net, hex.EncodeToString(pubkey.SerializeCompressed()))
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("%s", msg), nil
