...
func bindFlags() error {
	...
	pf.Int64("mintxfee", uc.MinTxFee, "The minimum transaction fee default 20000 (aka. 0.0002 MEER/KB)")
	pf.StringArray("apis", uc.APIs, "enabled APIs")

	pf.StringP("qserver", "S", uc.QServer, "btp node server, overwritten by btpdselect")
	pf.StringP("quser", "U", uc.QUser, "btp node username")
	pf.StringP("qpass", "P", uc.QPass, "btp node password")
	pf.String("bcert", uc.QCert, "Certificate path")
	pf.Bool("qnotls", uc.QNoTLS, "disable TLS")
	pf.Bool("qtlsskipverify", uc.QTLSSkipVerify, "skip TLS verification")
	pf.String("qproxy", uc.QProxy, "btp node proxy address")
	pf.String("qproxyuser", uc.QProxyUser, "btp node proxy username")
	pf.String("qproxypass", uc.QProxyPass, "btp node proxy password")
	pf.String("walletpass", uc.WalletPass, "data encryption password")
	pf.String("btpdselect", uc.BtpdSelect,
		"select btp RPC config defined in Btpds section of config file, overwrite qserver")
	...
	viper.SetDefault("BtpdSelect", dc.BtpdSelect)
	viper.SetDefault("Btpds", dc.Btpds)
	...
	viper.BindPFlag("QCert", pf.Lookup("bcert"))
	...
	viper.BindPFlag("BtpdSelect", pf.Lookup("btpdselect"))
	return nil
}
