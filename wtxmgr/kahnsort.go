...
func makeGraph(set map[hash.Hash]*types.Transaction) hashGraph {
	...
	inputLoop:
		for _, input := range tx.TxIn {
			...
			// edges.
			if _, ok := set[input.PreviousOut.Hash]; !ok {
				continue
			}

			inputNode := graph[input.PreviousOut.Hash]

			// Skip duplicate edges.
			for _, outEdge := range inputNode.outEdges {
				if *outEdge == input.PreviousOut.Hash {
					continue inputLoop
				}
			}
			...
			if inputTx == nil {
				inputTx = set[input.PreviousOut.Hash]
			}
			graph[input.PreviousOut.Hash] = graphNode{
				value:    inputTx,
...
func graphRoots(graph hashGraph) []*types.Transaction {
	roots := make([]*types.Transaction, 0, len(graph))
