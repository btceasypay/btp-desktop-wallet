...
import (
	...
	util "github.com/btceasypay/btp-desktop-wallet/utils"
	"github.com/btceasypay/btp-desktop-wallet/wallet/txrules"

	"github.com/btceasypay/bitcoinpay/core/types"
	"github.com/btceasypay/btp-desktop-wallet/json/btpjson"
	waddrmgr "github.com/btceasypay/btp-desktop-wallet/waddrmgs"
...
func CreateNewAccount(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.CreateNewAccountCmd)

	// The wildcard * is reserved by the rpc server with the special meaning
	// of "all accounts", so disallow naming accounts to this string.
	if cmd.Account == "*" {
		return nil, &btpjson.ErrReservedAccountName
	}

	_, err := w.NextAccount(waddrmgr.KeyScopeBIP0044, cmd.Account)
	if waddrmgr.IsError(err, waddrmgr.ErrLocked) {
		return nil, &btpjson.RPCError{
			Code: btpjson.ErrRPCWalletUnlockNeeded,
...
func GetNewAddress(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.GetNewAddressCmd)
...
func GetAddressesByAccount(iCmd interface{}, w *wallet.Wallet) ([]string, error) {
	cmd := iCmd.(*btpjson.GetAddressesByAccountCmd)
...
func GetAccountByAddress(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.GetAccountCmd)
	...
	if err != nil {
		return nil, &btpjson.ErrAddressNotInWallet
	}

	acctName, err := w.AccountName(waddrmgr.KeyScopeBIP0044, account)
	if err != nil {
		return nil, &btpjson.ErrAccountNameNotFound
	}
	return acctName, nil
}
...
func DumpPrivKey(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.DumpPrivKeyCmd)
	...
	if waddrmgr.IsError(err, waddrmgr.ErrLocked) {
		...
		// accessible.
		return nil, &btpjson.ErrWalletUnlockNeeded
	}
	return key, err
}
...
func SendToAddress(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.SendToAddressCmd)

	// Transaction comments are not yet supported.  Error instead of
	// pretending to save them.
	if !isNilOrEmpty(cmd.Comment) || !isNilOrEmpty(cmd.CommentTo) {
		return nil, &btpjson.RPCError{
			Code:    btpjson.ErrRPCUnimplemented,
	...
	if amt < 0 {
		return nil, btpjson.ErrNeedPositiveAmount
	}
...
func UpdateBlock(iCmd interface{}, w *wallet.Wallet) error {
	cmd := iCmd.(*btpjson.UpdateBlockToCmd)
...
func GetBalance(iCmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := iCmd.(*btpjson.GetBalanceByAddressCmd)
...
func GetListTxByAddr(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*btpjson.GetListTxByAddrCmd)
...
func GetBillByAddr(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*btpjson.GetBillByAddrCmd)
