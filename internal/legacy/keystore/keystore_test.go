...
import (
	...
	btcec "github.com/btceasypay/bitcoinpay/crypto/ecc/secp256k1"
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// the next pubkeys.
		pubkeyUncompressed, err := btcec.ParsePubKey(nextPubUncompressedFromPub)
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := btcec.ParsePubKey(nextPubCompressedFromPub)
		if err != nil {
...
func TestWatchingWalletExport(t *testing.T) {
	...
	for i := range newAddrs {
		if newAddrs[i].Encode() != newWWAddrs[i].Encode() {
...
//
//	// import priv key
//	wif, err := util.NewWIF((*btcec.PrivateKey)(pk), tstNetParams, false)
//	if err != nil {
//		t.Fatal(err)
//	}
...
func TestImportScript(t *testing.T) {
	...
	if sinfo.Address().Encode() != address.Encode() {
	...
	if sinfo2.Address().Encode() != address.Encode() {
		t.Error("script address doesn't match entry.")
		return
	}
