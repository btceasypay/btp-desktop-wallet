...
import (
	...
	chainhash "github.com/btceasypay/bitcoinpay/common/hash"
	...
	qitec "github.com/btceasypay/bitcoinpay/crypto/ecc/secp256k1"
	"github.com/btceasypay/bitcoinpay/engine/txscript"
	chaincfg "github.com/btceasypay/bitcoinpay/params"
	...
	"github.com/btceasypay/btp-desktop-wallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := ecc.Secp256k1.PrivKeyFromBytes(privkey)
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]types.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]types.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]types.Address)
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (types.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ChangeAddress(bs *BlockStamp) (types.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() types.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a types.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a types.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *utils.WIF, bs *BlockStamp) (types.Address, error) {
	...
	pkh := hash.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (types.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(hash.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
type WalletAddress interface {
	// Address returns a types.Address for the backing address.
...
func (s *Store) ActiveAddresses() map[types.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[types.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the types.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]types.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (a *btcAddress) Address() types.Address {
	return a.address
}
...
// address returns a types.AddressScriptHash for a btcAddress.
func (sa *scriptAddress) Address() types.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []types.Address {
	return sa.addresses
}
